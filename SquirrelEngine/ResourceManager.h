		...
		inline void Reload(std::string path) {
			...
			if (resources.find(path) != resources.end()) {
				...
				if (!resources[path].expired()) {
					...
					T* newlyLoadedResource(T::Load(path));
					T oldResourceData = *(std::dynamic_pointer_cast<T, Resource>(resources[path].lock()));

					*(std::dynamic_pointer_cast<T, Resource>(resources[path].lock())) = *newlyLoadedResource;
